...
-- Finds the point in the direction vec2 at radius distance away, adjusted for image bounds
--[[function pointOnImageInDirectionAtRadius(image, direction, radius, origin)
    local dir = direction:normalize()
    local point = origin + dir * radius
    local x = math.floor(point.x)
    local y = math.floor(point.y)
    if x < 1 then
        x = 1
    elseif x > image.width then
        x = image.width
    end
    if y < 1 then
        y = 1
    elseif y > image.height then
        y = image.height
    end
    return vec2(x, y)
end]]
